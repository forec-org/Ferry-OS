...
void Processor::interpreteCall() {
	...
	case INSTR_CALLCORE:
	{
		...
		apiNameLen = mMemory->readHalfWord(mCore->getPcbCurr()->getPid(), strIndex);

		//mMemory->readBlock((BYTE*)apiName, strIndex + sizeof(int), apiNameLen);
		unsigned int apiNameStart = strIndex + sizeof(int);
		for (unsigned int index = 0; index < apiNameLen; ++index) {
			apiName[index] = mMemory->readByte(mCore->getPcbCurr()->getPid(), apiNameStart + index);
		}
	...
	case INSTR_CALL:
	{
		...
		funcEntry = mMemory->readHalfWord(mCore->getPcbCurr()->getPid(), mFS + funcIndex * 8);
		funcLocalSize = mMemory->readHalfWord(mCore->getPcbCurr()->getPid(), mFS + funcIndex * 8 + 4);
...
int Processor::getStackValue(int stackIndex) {
	...
	if (stackIndex >= 0) {				//全局变量
		...
		value = mMemory->readHalfWord(mCore->getPcbCurr()->getPid(), mSS + stackIndex);
	}
	else if (stackIndex < 0) {			//局部变量
		//value = mMemory->readWord(mSS + (mBP + stackIndex)); 
		value = mMemory->readHalfWord(mCore->getPcbCurr()->getPid(), mSS + (mBP + stackIndex));
	}
	return value;
}
...
void Processor::setStackValue(int stackIndex, int value) {
	if (stackIndex >= 0) {				//全局变量
		...
		mMemory->write(mSS + stackIndex, (void*)&value, sizeof(int), mCore->getPcbCurr()->getPid());
	}
	else if (stackIndex < 0) {			//局部变量
		//mMemory->writeWord(mSS + (mBP + stackIndex), value);
		mMemory->write(mSS + (mBP + stackIndex), (void*)&value, sizeof(int), mCore->getPcbCurr()->getPid());
	}
}
...
void Processor::fetchIstr() {
	...
	for (unsigned int index = 0; index < INSTR_LEN; ++index) {
		IR[index] = mMemory->readByte(mCore->getPcbCurr()->getPid(), mCS + mPC + index);
	}
...
void Processor::exec() {
	while (1) {
		...
		mCore->getPcbCurr()->incTotalCycles();	//统计进程执行的CPU周期数
...
void Processor::push(int num) {
	...
	mMemory->write(mSS + mSP - 3, (void*)&num, sizeof(int), mCore->getPcbCurr()->getPid());
	mSP -= 4;
}

void Processor::pop(int & num) {
	//num = mMemory->readWord(mSS + mSP + 1);
	num = mMemory->readHalfWord(mCore->getPcbCurr()->getPid(), mSS + mSP + 1);
	mSP += 4;
}
